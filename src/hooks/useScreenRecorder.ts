...
export function useScreenRecorder(): UseScreenRecorderReturn {
	...
	const startRecording = async () => {
		...
		try {
			...
			// Chromium reports capture failures (e.g. missing PipeWire portal on
			// Linux/Wayland) as this opaque NotReadableError. Point the user at
			// the portal so the failure is actionable.
			const message =
				/could not start video source/i.test(rawMessage)
					? "Failed to start recording: could not start video source. Make sure a screen-sharing portal is running (PipeWire and xdg-desktop-portal with your desktop's backend, e.g. xdg-desktop-portal-gnome, -kde, or -wlr), then try again."
					: `Failed to start recording: ${rawMessage}`;
			alert(message);
			setRecording(false);
			if (nativeScreenRecording.current) {
				await discardActiveNativeCapture();
