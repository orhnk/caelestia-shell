...
function shouldForceSoftwareRendering(env: NodeJS.ProcessEnv): boolean {
	return env.RECORDLY_FORCE_SOFTWARE_RENDERING === "1";
}
...
export function getGpuSwitches(
	...
	if (platform === "linux" && shouldForceSoftwareRendering(env)) {
		return {
			useGl: "swiftshader",
