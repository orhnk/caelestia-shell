...
import { persistPendingCursorTelemetry } from "../cursor/telemetry";
import {
	currentProjectPath,
...
export function registerProjectHandlers() {
	...
	ipcMain.handle(
		...
		async (
			...
			// Persist any pending cursor telemetry for this video so the editor can
			// load it (the set-recording-state handler snapshots but cannot persist
			// because the video path is not yet known at that point).
			await persistPendingCursorTelemetry(normalizedVideoPath).catch(() => {});
			setCurrentRecordingSession({
